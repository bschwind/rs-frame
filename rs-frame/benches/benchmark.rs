...
fn trivial_benchmark(c: &mut Criterion) {
    c.bench_function("UsersListPath", |b| b.iter(|| {
...
fn simple_benchmark(c: &mut Criterion) {
    c.bench_function("UserDetailPath", |b| b.iter(|| {
...
fn vec_benchmark(c: &mut Criterion) {
    c.bench_function("UserDetailVecQueryPath", |b| b.iter(|| {
        let _path: UserDetailVecQueryPath = "/users/1024?friend_ids[1]=20&friend_ids[2]=33&friend_ids[0]=1".parse().unwrap();
    }));
}

criterion_group!(benches, trivial_benchmark, simple_benchmark, nested_benchmark, vec_benchmark);
